...
05-12-2020
Update 1: 07-12-2020 (+ Remember the last location entered)
'''
...
def launch_button_change(opt):
	...
	options=[['Launch','RoyalBlue2','launch',''],['Close','red','lambda:on_close_cap()',inst]]
...
def on_close_cap(): #Edited v1.1
	global cap
	cap.destroy()
	launch_button_change(0)

def on_close_main(): #Added v1.1
	global root,save_loc,transp
	with open(os.path.join(wd,'pref.txt'),'w') as file:
		file.truncate(0)
		file.write(f'{save_loc.get()}\n{transp.get()}')
		file.close()
	root.destroy()
...
def launch():
	...
	if save_loc.get():
		...
		cap.protocol("WM_DELETE_WINDOW",on_close_cap) #Edited v1.1
...
root.protocol("WM_DELETE_WINDOW",on_close_main) #Added v1.1
...
#Init #Added v1.1
wd=os.path.join(os.environ['APPDATA'],'CaptureA') 
if not os.path.exists(wd):
    os.mkdir(wd)
try:
	with open(os.path.join(wd,'pref.txt'),'r') as file: 
